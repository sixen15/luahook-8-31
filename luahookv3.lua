...
do
    if getgenv and getgenv().__LH_SetmtBP ~= game and hookfunction and newcclosure and getrenv then
        ...
        local ok = pcall(function()
            local oldsetmt
            local l1 = { hits = 0, weak = 0, len4 = 0, slot1 = 0, slot4 = 0 }
            getgenv().__LH_L1 = l1
            local rawget_, rawlen_, type_ = rawget, rawlen, type
            oldsetmt = hookfunction(getrenv().setmetatable, newcclosure(function(t, mt)
                if type_(mt) == "table" and rawget_(mt, "__mode") == "kv" and type_(t) == "table" then
                    l1.weak = l1.weak + 1
                    if rawlen_(t) == 4 then
                        l1.len4 = l1.len4 + 1
                        if type_(rawget_(t, 1)) == "table" then l1.slot1 = l1.slot1 + 1 end
                        if type_(rawget_(t, 4)) == "userdata" then l1.slot4 = l1.slot4 + 1 end
                        if rawget_(t, 2) == 1 and rawget_(t, 3) == "String" then
                            l1.hits = l1.hits + 1
...
do
    if getgenv then
        local g = getgenv()
        if type(g.LH) == "table" and type(g.LH.unload) == "function" then
            pcall(g.LH.unload)
        end
        g.__LH_GEN = (tonumber(g.__LH_GEN) or 0) + 1
        LH_GEN = g.__LH_GEN
    end
end
local function lhStale()
    if not getgenv then return false end
    local g = getgenv().__LH_GEN
    return g ~= nil and g ~= LH_GEN
end
...
local State = {
    ...
    CapabilityErrors = 0,
    CapabilityLastError = "",
    CapabilityElevateFails = 0,
    RageGlueWritePath = nil,
    RageGlueProbes    = 0,
    RageFPDHValue     = nil,
    CapabilitySilentRefusals = 0,
    CapabilityLastSilent     = "",
    CapabilityRepairs        = 0,
    IdentityClamped          = 0,
    ...
    RageReadyDisagree = 0,
    RageSwapGaveUp = 0,
    RageMeleePhase = "reset",
    RageBaitHoldFrames = 0,
    RageBaitPinRefused = 0,
    RageBaitRingFallbacks = 0,
    RageBaitWriteRefused = 0,
    RageVoidFires = 0,
    RageTransportNotice = nil,
    RagePoisonBlips = 0,
    RagePreFires = 0,
    RagePredTarget = nil,
...
local identityIsSafe = (function()
    ...
    pcall(function()
        game:GetService("LogService").MessageOut:Connect(function(msg)
            ...
            local now = tick()
            local gap = State.CapabilityLastAt and (now - State.CapabilityLastAt) or nil
            State.CapabilityLastAt = now
            if (State.CapabilityErrors or 0) <= 12 then
                print("[LuaHook] CAPABILITY ERROR #" .. tostring(State.CapabilityErrors)
                      .. (gap and ("   +" .. string.format("%.1f", gap) .. "s since the last one") or "   (first)")
                      .. "   [breadcrumb: " .. tostring(State.CapabilitySeam or "none")
                      .. " — NOT attribution]")
                local moved = {}
                pcall(function()
                    for k, v in pairs(State) do
                        if type(v) == "number" and v == v then
                            local d = v - (_capSnap[k] or 0)
                            _capSnap[k] = v
                            if d > 0 then moved[#moved + 1] = { k, d } end
                        end
                    end
                    table.sort(moved, function(a, b) return a[2] < b[2] end)
                end)
                if #moved > 0 then
                    local line = ""
                    for i = 1, math.min(8, #moved) do
                        line = line .. moved[i][1] .. "+" .. tostring(moved[i][2]) .. "  "
                    end
                    print("[LuaHook] ★ SMALLEST MOVERS since the previous error: " .. line)
                    print("[LuaHook] ★ a counter that reads +1 here on EVERY error sits next to the emitter.")
                end
            end
            if not killed then
                killed = true
...
local _capShown = {}
local function capPcall(seam, fn, a, b, c)
    local ok, res = pcall(fn, a, b, c)
    if ok then return true, res end
    local msg = tostring(res)
    if string.find(msg, "lacking capability", 1, true)
       or string.find(msg, "cannot access", 1, true) then
        State.CapabilityErrors = (State.CapabilityErrors or 0) + 1
        State.CapabilitySeam = seam
        State.CapabilityLastError = msg
        State.CapabilityLastStatus = tostring(State.RageStatus)
        State.CapabilityDirty = true
        if _capShown[seam] == nil then
            _capShown[seam] = true
            print("[LuaHook] ★ CAPABILITY REFUSAL CAUGHT — seam: " .. seam)
            print("[LuaHook] ★ engine said: " .. msg)
            print("[LuaHook] ★ rage status at the time: " .. tostring(State.RageStatus))
        end
    end
    return false, res
end
local _identBaseline = nil
pcall(function()
    local g = getthreadidentity or get_thread_identity or getidentity or getthreadcontext
    if type(g) == "function" then
        local v = g()
        if type(v) == "number" then _identBaseline = v end
    end
end)
State.IdentityBaseline = _identBaseline
local function identRestore(setter, prev)
    if type(setter) ~= "function" then return end
    local want = prev
    if type(want) ~= "number" then want = _identBaseline end
    if _identBaseline ~= nil and want ~= nil and want < _identBaseline then
        State.IdentityClamped = (State.IdentityClamped or 0) + 1
        want = _identBaseline
    end
    if want == nil then return end
    pcall(setter, want)
end
local _idGet, _idSet, _idTried = nil, nil, false
local function asidentity(n, fn, a, b, c)
    if not _idTried then
        _idTried = true
        pcall(function()
            _idGet = getthreadidentity or get_thread_identity
            _idSet = setthreadidentity or set_thread_identity or setidentity or setthreadcontext
        end)
    end
    if type(_idGet) == "function" and type(_idSet) == "function" and identityIsSafe() then
        local ok, prev = pcall(_idGet)
        if ok then
            pcall(_idSet, n)
            local done, res = pcall(fn, a, b, c)
            identRestore(_idSet, prev)
            if done then return true, res end
        end
    end
...
;(function()
    ...
    local function killFloor()
        local v = State.RageFPDHValue
        if type(v) ~= "number" or v ~= v then v = -500 end
        return v + Config.RageKillPlaneBuffer
    end
    ...
    local function startTargetLoop()
        ...
        _tgtConn = RunService.Heartbeat:Connect(function()
            if lhStale() then if _tgtConn then _tgtConn:Disconnect() end return end
    ...
    local function startLabPoll()
        ...
        _labConn = RunService.Heartbeat:Connect(function()
            if lhStale() then if _labConn then _labConn:Disconnect() end return end
    ...
    local function weaponReady(it)
        ...
        local okA, ammo = capPcall("07b weaponReady it:Get(Ammo)",
                                   function() return it:Get("Ammo") end)
        if not okA then
    ...
    local RELOAD_GIVEUP = 8
    local _reloadTries, _reloadFor = 0, nil
    Rage._ensureReload = function(it)
        if it == nil then return "no item" end
        if _reloadFor ~= it then _reloadFor, _reloadTries = it, 0 end
        local okR, reloading = pcall(function() return (it._reload_cooldown or 0) > tick() end)
        if okR and reloading then _reloadTries = 0; return "waiting" end
        local okA, ammo = capPcall("07b _ensureReload it:Get(Ammo)",
                                   function() return it:Get("Ammo") end)
        if not okA then
            State.RageAmmoUnreadable = (State.RageAmmoUnreadable or 0) + 1
            _reloadTries = 0
        ...
        if type(ammo) == "number" and ammo > 0 then _reloadTries = 0; return "loaded" end
        if _reloadTries >= RELOAD_GIVEUP then
            State.RageReloadGaveUp = (State.RageReloadGaveUp or 0) + 1
            return "loaded"
        end
    ...
    local function equipAtId2(lf, slot)
        task.spawn(function()
            asidentity(2, function() lf.EquipItem(lf, slot) end)
        end)
    end
    ...
    Rage._weaponRecovery = function(it)
        ...
        local okA, ammo = capPcall("07b _weaponRecovery it:Get(Ammo)",
                                   function() return it and it:Get("Ammo") end)
        if not okA then
            State.RageAmmoUnreadable = (State.RageAmmoUnreadable or 0) + 1
            return Rage._ensureReload(it)
        end
        local empty = not (type(ammo) == "number" and ammo > 0)
        ...
        if _swapFor ~= it then _swapFor, _swapTries = it, 0 end
        if _swapTries >= SWAP_GIVEUP then
            State.RageSwapGaveUp = (State.RageSwapGaveUp or 0) + 1
            return Rage._ensureReload(it)
        end
        ...
        task.spawn(function()
            ...
            EF.Play = function(self, victim, eliminator, ...)
                ...
                pcall(function()
                    ...
                    State.RageTransportNotice = (State.RageAutoTransport == "render") and "render" or "kerp"
                end)
    ...
    local function polarFire(eyePos, aimPos, hh)
        ...
        task.spawn(function()
            ...
            asidentity(2, function() lf:Input("StartShooting") end)
        end)
    end
    ...
    local function startRage()
        ...
        if not _rageCooldownConn then
            _rageCooldownConn = RunService.Heartbeat:Connect(function()
                if lhStale() then if _rageCooldownConn then _rageCooldownConn:Disconnect() end return end
                pcall(defeatCooldowns)
            end)
        end
        _rageConn = RunService.Heartbeat:Connect(function()
            if lhStale() then if _rageConn then _rageConn:Disconnect() end return end
            ...
            if State.RageTransportNotice ~= nil then
                local notice = State.RageTransportNotice
                State.RageTransportNotice = nil
                local lib = _G["\76\72"]
                if lib and lib.Notify then
                    pcall(function()
                        lib:Notify({
                            Title = "Park (Auto)",
                            Description = "Switched to " .. notice,
                            Time = 4,
                        })
                    end)
                end
...
;(function()
    ...
    local _deflectSince = setmetatable({}, { __mode = "k" })
    local _shots      = 0
    local _voidSteps  = 0
    ...
    local function setPhysicsFlags(on)
        ...
        if _fpdhOriginal == nil then
            _fpdhOriginal = select(2, asidentity(8, function() return workspace.FallenPartsDestroyHeight end))
            if type(_fpdhOriginal) ~= "number" or _fpdhOriginal ~= _fpdhOriginal then _fpdhOriginal = -500 end
            State.RageFPDHValue = _fpdhOriginal
        end
    ...
    Rage._identRaise = function()
        if not identEnsure() then return false end
        if not identityIsSafe() then return false end
        if _identSet == nil then return false end
        local ok = pcall(_identSet, 8)
        if ok then State.CapabilityRepairs = (State.CapabilityRepairs or 0) + 1 end
        return ok
    end
    Rage._capSilent = function(what, fn)
        State.CapabilitySilentRefusals = (State.CapabilitySilentRefusals or 0) + 1
        State.CapabilityLastSilent = what
        State.CapabilityDirty = true
        local shown = Rage._capSilentShown
        if shown == nil then shown = {}; Rage._capSilentShown = shown end
        if shown[what] == nil then
            shown[what] = true
            print("[LuaHook] ★ SILENT CAPABILITY REFUSAL (S64) — " .. what)
            print("[LuaHook] ★ the call did not throw and did not do anything. That is the logged-not-raised")
            print("[LuaHook] ★ refusal, which is why every pcall in this bundle reported success.")
            print("[LuaHook] ★ raising this thread to identity 8 and retrying the walk in place.")
        end
        Rage._identRaise()
    ...
    local function rawSetCFrame(hrp, cf)
        ...
        return capPcall("07c park write (bare fallback, elevation unavailable)",
                        function() hrp.CFrame = cf end)
    end
    local _rvCF = nil
    ...
    local _strand = { map = setmetatable({}, { __mode = "k" }) }
    local _gluePrev   = nil
    ...
    _strand.capture = function(hit)
        local w = hit:FindFirstChildOfClass("WeldConstraint") or hit:FindFirstChild("WeldConstraint")
        local a = w and w.Part1
        if a == nil or a.Parent == nil then
            a = hit.Parent and hit.Parent:FindFirstChild("HumanoidRootPart") or nil
        end
        if a == nil then return end
        pcall(function() _strand.map[hit] = { a, a.CFrame:ToObjectSpace(hit.CFrame) } end)
    end
    _strand.restore = function(hit)
        if hit == nil or hit.Parent == nil then return false end
        local rec = _strand.map[hit]
        if rec == nil then return false end
        local a, rel = rec[1], rec[2]
        if a == nil or a.Parent == nil then return false end
        local ok = pcall(function() hit.CFrame = a.CFrame * rel end)
        local p = hit.Position
        if ok and p == p and math.abs(p.X) < SANE_POS_LIMIT
           and math.abs(p.Y) < SANE_POS_LIMIT and math.abs(p.Z) < SANE_POS_LIMIT then
            _strand.map[hit] = nil
            return true
        end
        State.RageGlueStrand = (State.RageGlueStrand or 0) + 1
        return false
    end
        ...
        for _, p in Players:GetPlayers() do
            if p ~= lp then
                ...
                if ref ~= nil and isSanePos(ref.Position) then
                    if hh ~= nil and not mine and isSanePos(hh.Position) then _predLast[p] = hh.Position end
    ...
    local function killFloor()
        local val = State.RageFPDHValue
        if type(val) == "number" and val == val then return val + KILL_PLANE_BUF end
        return -400
    end
    local function envIdOf(player)
        local id = nil
    ...
    local function weaponReady(it)
        ...
        local okA, ammo = capPcall("07c weaponReady it:Get(Ammo)",
                                   function() return it:Get("Ammo") end)
        if not okA then
            State.RageAmmoUnreadable = (State.RageAmmoUnreadable or 0) + 1
            return true
    ...
    local function meleeProfile(it)
        ...
        pcall(function()
            local info = it.Info
            if info == nil then return end
            if not (Rage._itemIsMelee and Rage._itemIsMelee(it)) then return end
    ...
    local function meleeStrike(firePark, hpos, hh, tgt)
        ...
        if not glueActive() and not RageKit.swingIsOpen() then
            State.RageKnifeStatus = "cycle (quiet)"
            return false
        end
        ...
        local vp, vy, vr = trp.CFrame:ToOrientation()
        ...
        local lf = Rivals.Fighter and Rivals.Fighter.LocalFighter
        if not sent then
            if Config.RageMeleeAsk ~= false and not glueActive() then
                ...
                if _useItem == nil then
                    capPcall("07c meleeStrike UseItem remote walk",
                             function() _useItem = ReplicatedStorage.Remotes.Replication.Fighter.UseItem end)
                    if _useItem == nil then
                        Rage._capSilent("meleeStrike UseItem remote walk",
                                        function() _useItem = ReplicatedStorage.Remotes.Replication.Fighter.UseItem end)
                    end
                end
                local okId, objId = capPcall("07c meleeStrike it:Get(ObjectID)",
                                             function() return it:Get("ObjectID") end)
                ...
                local fok = atId2(function()
                    local inner = {}
                    if glueActive() then
                        inner[utf8.char(0)] = RageKit.gluePose(GLUE_CHAR0, vy, vr)
                        inner[utf8.char(1)] = RageKit.gluePose(GLUE_CHAR1, vy, vr)
    ...
    local function polarFire(eyePos, aimPos, hh, taps)
        ...
        if _shootEnum == nil then
            capPcall("07c polarFire StartShooting enum walk",
                     function() _shootEnum = Rivals.Enums:ToEnum("StartShooting") end)
            if _shootEnum == nil then
                Rage._capSilent("polarFire StartShooting enum walk",
                                function() _shootEnum = Rivals.Enums:ToEnum("StartShooting") end)
            end
        end
        if _shootEnum == nil then _fireWhy = "StartShooting enum unresolved"; return 0 end
        if _useItem == nil then
            capPcall("07c polarFire UseItem remote walk",
                     function() _useItem = ReplicatedStorage.Remotes.Replication.Fighter.UseItem end)
            if _useItem == nil then
                Rage._capSilent("polarFire UseItem remote walk",
                                function() _useItem = ReplicatedStorage.Remotes.Replication.Fighter.UseItem end)
            end
        end
        if _useItem == nil then _fireWhy = "UseItem remote unresolved (capability?)"; return 0 end
        local okId, objId = capPcall("07c polarFire it:Get(ObjectID)",
                                     function() return it:Get("ObjectID") end)
        if okId and objId == nil then
            Rage._capSilent("polarFire it:Get(ObjectID)",
                            function() okId, objId = true, it:Get("ObjectID") end)
        end
    ...
    local function polarTick(ch, hrp)
        ...
        if Config.AvoidDeflect and isDeflecting(tgt) then
            local now = tick()
            if _deflectSince[tgt] == nil then _deflectSince[tgt] = now end
            if now - _deflectSince[tgt] < DEFLECT_MAX_HOLD then return hide(hrp, "Deflecting") end
        else
            _deflectSince[tgt] = nil
        end
        local it = getEquippedItem()
        if not weaponReady(it) then
            ...
            if act == "loaded" then
                State.RageReadyDisagree = (State.RageReadyDisagree or 0) + 1
            else
        ...
        local refPos = hpos
        if trp ~= nil then refPos = trp.Position end
        if trp ~= nil and not isSanePos(hpos) and isSanePos(refPos) then
            _strand.restore(hh)
            hpos = hh.Position
            if not isSanePos(hpos) then
                hpos = trp.Position + Vector3.new(0, STRAND_HEAD_UP, 0)
                State.RageStrandRoutes = (State.RageStrandRoutes or 0) + 1
            end
        end
        local predicting = false
        local voidFire   = false
        local rv         = nil
        if not isSanePos(refPos) then
            local pre = nil
        ...
        local aimPos = hpos
        if rv == nil and not predicting then rv = glueAcquire(hh) end
        ...
        if melee and not RageKit.swingOpen(it, tgt) and rv == nil then
            return hide(hrp, "Melee (cycle)")
        end
        ...
        if translocateCF ~= nil and not parked then           _baitGateWhy = "dip write REFUSED"
        elseif startingTranslocate and translocateCF ~= nil then _baitGateWhy = "FIRING"
        elseif _translocateHoldCF ~= nil then                 _baitGateWhy = "DIPPING (holding)"
        ...
        if translocateCF ~= nil then
            ...
            if parked then
                ...
                _translocateHoldUntil, _translocateHoldCF = 0, nil
                State.RageBaitWriteRefused = (State.RageBaitWriteRefused or 0) + 1
    ...
    function PolarCore.start()
        ...
        _charConn = lp.CharacterAdded:Connect(function()
            ...
            _prevPark, _prevParkTgt = nil, nil
            _parkFailStreak = 0
            pcall(RageKit.phaseReset)
            _lastPark = nil
            _hiding, _primeUntil = true, 0
        end)
        _conn = RunService.Heartbeat:Connect(function(dt)
            if lhStale() then if _conn then _conn:Disconnect() end return end
            State.RageTranslocating = false
            tickHackers(dt or 0)
            tickPredict()
    ...
    function Rage._physDiag()
        ...
        print("[LuaHook] FPDH:   " .. tostring(State.RageFPDHPath)
              .. "   cached=" .. tostring(State.RageFPDHValue))
        ...
        print("[LuaHook] gum: mode=" .. tostring(gumMode())
              ...
              .. "  writePath=" .. tostring(State.RageGlueWritePath or "unmeasured")
              .. " probes=" .. tostring(State.RageGlueProbes or 0)
        ...
        print("[LuaHook] ★ CAPABILITY, MEASURED (S64): silentRefusals="
              .. tostring(State.CapabilitySilentRefusals or 0)
              .. " (" .. tostring(State.CapabilityLastSilent or "none") .. ")"
              .. "  threadRepairs=" .. tostring(State.CapabilityRepairs or 0)
              .. "  identityClamped=" .. tostring(State.IdentityClamped or 0)
              .. "  baseline=" .. tostring(State.IdentityBaseline))
        print("[LuaHook] ★ CAPABILITY, MEASURED (S63): refusal=" .. tostring(State.CapabilitySeam or "<none>")
              .. "  elevateFails=" .. tostring(State.CapabilityElevateFails or 0)
              .. "  reloadGaveUp=" .. tostring(State.RageReloadGaveUp or 0)
              .. "  swapGaveUp=" .. tostring(State.RageSwapGaveUp or 0))
        local capMsg = State.CapabilityLastError
        if type(capMsg) == "string" and capMsg ~= "" then
            print("[LuaHook] ★ engine said: " .. capMsg)
        else
            print("[LuaHook] ★ engine said: <nothing caught> — no gated call in the fire path threw")
        end
...
RageKit = {}
;(function()
    RageKit.gluePose = function(base, yaw, roll)
        if type(base) ~= "table" then return base end
        if type(yaw) ~= "number" or type(roll) ~= "number" then return base end
        if yaw ~= yaw or roll ~= roll then return base end
        return {
            [utf8.char(0)] = base[utf8.char(0)],
            [utf8.char(1)] = base[utf8.char(1)],
            [utf8.char(2)] = base[utf8.char(2)],
            [utf8.char(3)] = base[utf8.char(3)],
            [utf8.char(4)] = yaw,
            [utf8.char(5)] = roll,
        }
    end
    local PHASE_SETTLE = 1 / 15
    local PHASE_MIN    = 0.45
    local WIDEN_AFTER  = 4
    local _phaseAt    = -1
    local _phaseCyc   = 0
    local _phaseMiss  = 0
    local _phaseOpen  = true
    local _phaseOpenAt = -1
    local _phaseSentAt = -1
    local _phaseHp    = nil
    local _phaseTgt   = nil
    local function cycleOf(it)
        local c = nil
        pcall(function()
            local info = it.Info
            if info == nil then return end
            if info.CriticalDamage ~= nil and type(info.HeavyAttackCooldown) == "number" then
                c = info.HeavyAttackCooldown
            else
                c = info.AttackCooldown
            end
        end)
        if type(c) ~= "number" or c ~= c or c <= 0 then return nil end
        return c
    end
    local function sawDamage(tgt)
        local hp = nil
        pcall(function()
            local ch = tgt and tgt.Character
            local hum = ch and ch:FindFirstChildOfClass("Humanoid")
            if hum ~= nil then hp = hum.Health end
        end)
        if type(hp) ~= "number" or hp ~= hp then return false end
        if _phaseTgt ~= tgt then
            _phaseTgt, _phaseHp = tgt, hp
            return false
        end
        local prev = _phaseHp
        _phaseHp = hp
        return type(prev) == "number" and hp < prev - 0.5
    end
    RageKit.phaseReset = function()
        _phaseAt, _phaseCyc, _phaseMiss = -1, 0, 0
        _phaseOpen = true
        _phaseOpenAt, _phaseSentAt = -1, -1
        _phaseHp, _phaseTgt = nil, nil
        State.RageMeleePhase = "reset"
    end
    RageKit.swingSent = function()
        if _phaseOpenAt < 0 then return end
        if _phaseSentAt >= 0 then return end
        _phaseSentAt = tick()
    end
    RageKit.swingOpen = function(it, tgt)
        local cyc = cycleOf(it)
        if cyc == nil or cyc < PHASE_MIN then
            _phaseOpen = true
            State.RageMeleePhase = "always"
            return true
        end
        if sawDamage(tgt) then _phaseMiss = 0 end
        if _phaseMiss >= WIDEN_AFTER then
            _phaseOpen = true
            State.RageMeleePhase = "wide"
            return true
        end
        local now = tick()
        if _phaseAt < 0 or cyc ~= _phaseCyc then
            _phaseAt, _phaseCyc = now, cyc
            _phaseOpenAt, _phaseSentAt = now, -1
            _phaseOpen = true
            State.RageMeleePhase = "seed"
            return true
        end
        if _phaseOpenAt >= 0 then
            local anchor = _phaseSentAt
            if anchor >= 0 and now > anchor + PHASE_SETTLE then
                _phaseAt = anchor + cyc
                _phaseOpenAt, _phaseSentAt = -1, -1
            elseif anchor < 0 and now > _phaseOpenAt + cyc then
                _phaseAt = now + cyc
                _phaseOpenAt = -1
                _phaseMiss = _phaseMiss + 1
            else
                _phaseOpen = true
                State.RageMeleePhase = "open"
                return true
            end
        end
        if now >= _phaseAt - PHASE_SETTLE then
            _phaseOpenAt, _phaseSentAt = now, -1
            _phaseMiss = _phaseMiss + 1
            _phaseOpen = true
            State.RageMeleePhase = "open"
        ...
        _phaseOpen = false
        State.RageMeleePhase = "quiet"
        return false
    end
    RageKit.swingIsOpen = function() return _phaseOpen end
end)()
    ...
    function Lab.enable()
        ...
        _conn = RunService.Heartbeat:Connect(function()
            ...
            _label.Text = string.format(
                ...
                .. "  dip held %d f   refused %d   ring %d   wrefz %d\n"
                .. "  NOFIRE %d f\n"
                .. "  ident %s   ammo? %d\n"
                .. "MELEE phase %s   %s\n"
                ...
                State.RageBaitRingFallbacks or 0, State.RageBaitWriteRefused or 0,
                State.RageFireZeroFrames or 0,
                tostring(State.IdentitySafe), State.RageAmmoUnreadable or 0,
                tostring(State.RageMeleePhase or "?"), tostring(State.RageKnifeStatus or "idle"),
...
;(function()
    ...
    local ok = pcall(function()
        getgenv().LH = {
            ...
            gen = function() return getgenv and getgenv().__LH_GEN or "unknown" end,
            why = function()
                local l1 = getgenv and getgenv().__LH_L1
                return "gen=" .. tostring(getgenv and getgenv().__LH_GEN or "?")
                    .. "  acLayer1Hits=" .. tostring(l1 and l1.hits or "NOT INSTALLED") .. " (want exactly 1)"
                    .. " [weak=" .. tostring(l1 and l1.weak or "?")
                    .. " len4=" .. tostring(l1 and l1.len4 or "?")
                    .. " slot1=" .. tostring(l1 and l1.slot1 or "?")
                    .. " slot4=" .. tostring(l1 and l1.slot4 or "?") .. "]"
...
do
    ...
    if getgenv and type(getgenv().LH) == "table" then getgenv().LH.unload = doUnload end
    L:AddButton({ Text = 'UNLOAD LuaHook', DoubleClick = true, Func = doUnload })
    UserInputService.InputBegan:Connect(function(input, gpe)
        if gpe then return end
